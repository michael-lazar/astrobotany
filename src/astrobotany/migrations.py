...
def add_item_paperclip(migrator):
    for user in User.select():
        ItemSlot.get_or_create(
            user=user,
            item_id=items.paperclip.item_id,
            defaults={"quantity": 1},
        )
...
def add_item_fertilizer(migrator):
    for user in User.select():
        ItemSlot.get_or_create(
            user=user,
            item_id=items.fertilizer.item_id,
            defaults={"quantity": 5},
        )
...
def add_user_password_field(migrator):
    migrate.migrate(
        migrator.add_column("user", "password", BlobField(null=True)),
    )
...
def move_ansi_enabled(migrator):
    migrate.migrate(
        migrator.add_column("certificate", "ansi_enabled", BooleanField(default=False)),
    )
...
def add_shaken_at(migrator):
    migrate.migrate(
        migrator.add_column("plant", "shaken_at", IntegerField(default=False)),
    )
...
def add_inbox_item(migrator):
    migrate.migrate(
        migrator.add_column("inbox", "item_id", IntegerField(null=True, default=None)),
    )
...
def add_badge_id(migrator):
    migrate.migrate(
        migrator.add_column("user", "badge_id", IntegerField(null=True, default=None)),
    )


def add_emoji_mode(migrator):
    migrate.migrate(
        migrator.add_column("certificate", "emoji_mode", IntegerField(default=0)),
    )
...
def add_pond(migrator):
    ...
    migrate.migrate(
        migrator.add_column("event", "count", IntegerField(default=0)),
    )
...
def add_watered_at_owner(migrator):
    dt = datetime.now()
    migrate.migrate(
        migrator.add_column("plant", "watered_at_owner", DateTimeField(default=dt)),
    )


def add_fence_active(migrator):
    migrate.migrate(
        migrator.add_column("user", "fence_active", BooleanField(default=False)),
    )
...
def main():
    ...
    parser.add_argument(
        "migration",
        help="The name the migration function to run",
    )
    parser.add_argument(
        "--db",
        default=settings.db,
